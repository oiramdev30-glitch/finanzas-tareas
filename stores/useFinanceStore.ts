...
import { computeBudgetSpent } from '../lib/finance';
...
export const useFinanceStore = create<FinanceState>()((set, get) => {
  ...
  function requireDeviceId(): Promise<string> {
    return getDeviceId();
  }
  ...
  return {
    ...
    totalBalance: () =>
      get()
        .accounts.filter((account) => account.include_in_balance && account.archived_at === null)
    ...
    getMonthSummary: (monthKey) => {
      ...
      for (const transaction of get().transactions) {
        ...
        if (monthKeyOf(transaction.date) !== monthKey) {
          continue;
        }
