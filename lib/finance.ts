...
function isOpeningBalance(transaction: AppTransaction): boolean {
  return (transaction as { is_opening_balance?: unknown }).is_opening_balance === true;
}
...
export function computeMonthSummary(
  ...
  for (const transaction of transactions) {
    ...
    if (isOpeningBalance(transaction)) continue;
...
export function computeWeekSummary(
  ...
  let expense = 0;
  for (const transaction of transactions) {
    if (transaction.type === 'transfer' || isOpeningBalance(transaction)) continue;
...
export function getMonthlyTrend(
  ...
  for (let i = monthsCount - 1; i >= 0; i -= 1) {
    ...
    points.push({
      key,
      label: new Date(year, month - 1, 1).toLocaleDateString('es-MX', { month: 'short' }),
